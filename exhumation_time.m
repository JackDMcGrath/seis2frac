function [thickness,fracture_time,ex]=exhumation_time(octcenters,quadcenters,exhum,seis_depths,seis_ix,OT,QT,plot_fig,fault,origin)
...
if plot_fig==1;
    ...
    for ii=1:OT.BinCount
        try
            fill(OT.BinCorners([cc],1,ii),OT.BinCorners([cc],2,ii),bamako(ceil(size(bamako,1)*(ex(ii)/10)),:))
        end
    end
    ...
    for ii=1:OT.BinCount
        try
            fill(OT.BinCorners(:,1,ii),OT.BinCorners(:,2,ii),bamako(ceil(size(bamako,1)*(fracture_time(ii)/5e6)),:))
            drawnow
        catch
            if ~isnan(fracture_time(ii))
                fill(OT.BinCorners(:,1,ii),OT.BinCorners(:,2,ii),bamako(size(bamako,1),:))
            end
        end
    end
