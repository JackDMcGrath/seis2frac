...
[DT,dropped3] = quadtree_subsample(eqd(dropped2,1:3),af,bearing,dip,QuadminbinCapacity,r_flg,ll_utm,'shrink',1,'centeredSquare',50);
...
%% Bin GR https://tinyurl.com/3s7tmyfe

bin_events=nan(QT.BinCount,1);
bin_MOC=nan(QT.BinCount,1);
bin_total=nan(QT.BinCount,1);
bin_area=nan(QT.BinCount,1);
bin_alpha=nan(QT.BinCount,1);
bin_center=nan(QT.BinCount,2);

for ii=1:QT.BinCount
    
    bin_eqs=eqs(QT.PointBins==ii,:); % Get events in one bin
    
    [bin_n,~,bin_alpha(ii),bin_btrend]=gutenberg_richter(bin_eqs(:,4),bins,bvalue,[],MOC,0); % Calculate GR trends for bin
    
    bin_events(ii)=size(bin_eqs,1);
    bin_MOC(ii)=length(find(bin_eqs(:,4)>=MOC));
    bin_total(ii)=10^bin_btrend(1);
    
    bin_area(ii)=prod([sqrt((QT.BinCorners(1,1,ii)-QT.BinCorners(2,1,ii))^2+(QT.BinCorners(1,2,ii)-QT.BinCorners(2,2,ii))^2), ...
        sqrt((QT.BinCorners(1,1,ii)-QT.BinCorners(4,1,ii))^2+(QT.BinCorners(1,2,ii)-QT.BinCorners(4,2,ii))^2)]);
